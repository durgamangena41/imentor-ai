    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={config ? 'Edit LLM Configuration' : 'Add New LLM'}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div><label className="block text-sm font-medium">Model ID</label><input name="modelId" value={formData.modelId} onChange={handleChange} className="input-field" placeholder="e.g., gemini-2.0-flash or ollama/qwen2" required /></div>
                <div><label className="block text-sm font-medium">Provider</label><select name="provider" value={formData.provider} onChange={handleChange} className="input-field"><option value="gemini">gemini</option><option value="ollama">ollama</option><option value="openai">openai</option><option value="claude">claude</option><option value="mistral">mistral</option><option value="fine-tuned">fine-tuned</option></select></div>
