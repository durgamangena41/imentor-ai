...
ENABLE_HEAVY_IMPORTS = os.getenv("ENABLE_HEAVY_IMPORTS", "false").lower() in ("1", "true", "yes", "on")

if ENABLE_HEAVY_IMPORTS:
    try:
        import whisper
        WHISPER_AVAILABLE = True
    except Exception:
        WHISPER_AVAILABLE, whisper = False, None
        logger.warning("Whisper/PyTorch not available. Audio transcription will be disabled.")
    ...
    WHISPER_AVAILABLE, whisper = False, None
    PLAYWRIGHT_AVAILABLE, sync_playwright = False, None
    logger.info("Heavy imports are disabled (ENABLE_HEAVY_IMPORTS=false).")
...
# Preloading these models can make startup very slow on some machines.
# Keep disabled by default and rely on lazy-loading paths in runtime services.
PRELOAD_MODELS = os.getenv("PRELOAD_MODELS", "false").lower() in ("1", "true", "yes", "on")
